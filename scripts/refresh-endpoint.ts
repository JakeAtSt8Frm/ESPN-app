 ...
 * It runs no user input — the command it runs is a constant, and the only thing
 * a caller controls is whether to also refit — so there is nothing here to
 * inject into. It must never be mounted on a public listener, which is why it
 * lives in `scripts/` and is wired only into the dev server and `npm run serve`.
 */
...
function runStep(step: RefreshStep): Promise<{ ok: boolean; output: string; ms: number }> {
 ...
 *
 * Two refreshes at once would have two processes writing the same JSON files,
 * and a client that reloaded between the two writes would read a snapshot half
 * from each. Joining is also the behaviour a person expects from pressing a
 * button twice.
 */
export function refresh(steps: RefreshStep[]): Promise<RefreshResult> {
  if (running) return running;

  running = (async (): Promise<RefreshResult> => {
    ...
    for (let i = 0; i < queue.length; i++) {
      const step = queue[i];
      const { ok, output, ms } = await runStep(step);
...
export async function handleRefresh(
  ...
  try {
    const result = await refresh(steps);
